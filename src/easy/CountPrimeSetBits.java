...
public class CountPrimeSetBits {
    public static void main(String[] args) {
        CountPrimeSetBits tester = new CountPrimeSetBits();
        System.out.println(tester.countPrimeSetBits(6, 10));
        System.out.println(tester.countPrimeSetBits(10, 15));
    }

    /*
        Algorithm:
        1. For Each Number in the given range
            1.a Convert the number to binary
            1.b count the number of 1's in the representation
            1.c check if the count is prime number
            1.d if prime increment resultCount
        2. return final count
     */
    private int countPrimeSetBits(int L, int R) {
        int counter = 0;
        for (int i = L; i<= R; i++){
            if(isPrime(count1sInBinaryRepFor(i))){
                counter++;
            }
        }
    ...
    private int count1sInBinaryRepFor(int number){
